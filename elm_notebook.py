...
import numpy as np
from pylab import plot, hist, mean, std, scatter
...
irx_train, irx_test, iry_train, iry_test = train_test_split(
    irx, iry, test_size=0.2)

digits = load_digits()
dgx, dgy = stdsc.fit_transform(digits.data / 16.0), digits.target
dgx_train, dgx_test, dgy_train, dgy_test = train_test_split(
    dgx, dgy, test_size=0.2)

diabetes = load_diabetes()
dbx, dby = stdsc.fit_transform(diabetes.data), diabetes.target
dbx_train, dbx_test, dby_train, dby_test = train_test_split(
...
tr, ts = res_dist(dgx, dgy, GenELMClassifier(
    hidden_layer=rbf_rhl), n_runs=100, random_state=0)
...
tr, ts = res_dist(dbx, dby, RandomForestRegressor(
    n_estimators=15), n_runs=100, random_state=0)
hist(tr), hist(ts)
print()
...
elmc = ELMClassifier(
    n_hidden=1000, activation_func='gaussian', alpha=0.0, random_state=0)
elmc.fit(dgx_train, dgy_train)
print(elmc.score(dgx_train, dgy_train), elmc.score(dgx_test, dgy_test))

# <codecell>

elmc = ELMClassifier(n_hidden=500, activation_func='hardlim',
